...
print("--------------------------------")
getRecommend = model.bestModel.recommendForAllUsers(15)
recommendations = getRecommend.withColumn("rec_exp", explode("recommendations")).select('userId', col("rec_exp.movieId"), col("rec_exp.rating"))

print("Top 15 movies recommendations for user id 10")
recommendations.filter("userId = 10").show()

print("Top 15 movies recommendations for user id 14")
recommendations.filter("userId = 14").show()
